...
// Same search order as the server, including the currently active Aside
// account rather than assuming the first account directory.
function asideRoot() {
  try {
    const accounts = JSON.parse(fs.readFileSync(expandHome('~/.aside/accounts.json'), 'utf8'));
    const id = Number(accounts.currentAccountId);
    if (Number.isInteger(id) && id >= 0) return expandHome(`~/.aside/u/${id}`);
  } catch {}
  return expandHome('~/.aside/u/0');
}
const configCandidates = process.env.MINIAPP_CONFIG
  ...
  : [
      ...
      path.join(asideRoot(), 'telegram-bridge/config.json'),
    ];
const configPath =
