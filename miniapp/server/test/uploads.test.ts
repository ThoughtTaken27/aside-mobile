...
describe('orphan upload cleanup', () => {
  it('removes expired regular files after a restart but keeps fresh bytes', async () => {
    const { sweepUploadDirectory } = await import('../src/uploads.js');
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'upload-sweep-'));
    try {
      const old = saveUpload(root, 'old.txt', Buffer.from('old'), 'text/plain', new Date(0));
      const fresh = saveUpload(root, 'fresh.txt', Buffer.from('fresh'));
      fs.utimesSync(old.path, new Date(0), new Date(0));
      expect(sweepUploadDirectory(root, 1000, Date.now())).toBe(1);
  ...
  it('never follows an unexpected symlink', async () => {
    const { sweepUploadDirectory } = await import('../src/uploads.js');
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'upload-sweep-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'upload-outside-'));
    try {
      const target = path.join(outside, 'keep.txt');
      fs.writeFileSync(target, 'keep');
      fs.symlinkSync(outside, path.join(root, 'linked-day'));
      sweepUploadDirectory(root, 0, Date.now());
