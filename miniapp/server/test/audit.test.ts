...
describe('A-2 the log serializer drops the query string', () => {
  /**
   * Fastify's default req serializer logged `req.url` whole -- which used
   * to write a live bearer into miniapp.log on every download carrying a
   * `?token=`. Query tokens are refused now, but the redaction stays.
   */
...
describe('B-9 upload receipts and their bytes are reclaimed', () => {
  ...
  it('deletes earlier multipart bytes when a later part makes the request fail', async () => {
    const auth = { authorization: `Bearer ${await authToken()}` };
    const boundary = '----partial-failure';
    let body = '';
    for (let i = 0; i < 6; i += 1) {
      body +=
        `--${boundary}\r\n` +
        `Content-Disposition: form-data; name="files"; filename="part-${i}.txt"\r\n` +
        'Content-Type: text/plain\r\n\r\nx\r\n';
    }
    body += `--${boundary}--\r\n`;
    const res = await app.inject({
    ...
    const remaining = fs.existsSync(env.uploadsDir)
      ? fs.readdirSync(env.uploadsDir, { recursive: true }).filter((name) => String(name).endsWith('.txt'))
      : [];
    expect(remaining).toEqual([]);
  });

  it('deletes the oldest bytes when the receipt cap evicts them', async () => {
    const auth = { authorization: `Bearer ${await authToken()}` };
    let first = '';
    let last = '';
    let issued = 0;
    // Five files per request stays well below the request-rate ceiling while
    // crossing the 200-receipt storage ceiling by exactly one file.
    for (let batch = 0; batch < 41; batch += 1) {
      const boundary = `----cap-${batch}`;
      const count = batch === 40 ? 1 : 5;
      let body = '';
      for (let item = 0; item < count; item += 1) {
        body +=
          `--${boundary}\r\n` +
          `Content-Disposition: form-data; name="files"; filename="probe-${issued}.txt"\r\n` +
          'Content-Type: text/plain\r\n\r\nx\r\n';
        issued += 1;
      }
      body += `--${boundary}--\r\n`;
      ...
      expect(up.statusCode).toBe(200);
      const saved = up.json().files as Array<{ path: string }>;
      if (batch === 0) first = saved[0].path;
      if (batch === 40) last = saved[0].path;
    }
...
describe('C-1 gaps surfaced by mutating the code under test', () => {
  ...
  it('never batches queued turns across different efforts', async () => {
    ...
    await tick(10);
    expect(argvs[1].at(-1)).toBe('first');
    expect(argvs[1]).not.toContain('--effort');
    children[1].emit('close', 0);
    await tick(10);
    expect(argvs[2].at(-1)).toBe('second');
    expect(argvs[2]).not.toContain('--effort');
    runner.shutdown();
  });
});
...
describe('A-3 the WebSocket upgrade is gated', () => {
  ...
  it('still lets the real client in', async () => {
    ...
    const first = await new Promise<any>((resolve) => {
      ...
      ws.on('open', () =>
        ws.send(JSON.stringify({ type: 'auth', token: res.json().token })),
      );
    });
  ...
  it('refuses query credentials on every protected route, even with valid auth', async () => {
    const auth = await live.inject({
      method: 'POST', url: '/api/auth', payload: { initDataRaw: initData() },
    });
    const headers = { authorization: `Bearer ${auth.json().token}` };
    for (const url of [
      '/api/status?token=abc',
      '/api/sessions/fixtureAAAA/artifacts/file?group=artifacts&path=x.md&token=abc',
    ]) {
      const res = await live.inject({ method: 'GET', url, headers });
      expect(res.statusCode).toBe(401);
      expect(res.json()).toEqual({ error: 'unauthorized', reason: 'token_in_query' });
    }
  });

  it('accepts the session cookie on the WebSocket upgrade', async () => {
    const auth = await live.inject({
      method: 'POST',
      url: '/api/auth',
      payload: { initDataRaw: initData() },
    ...
    const cookie = `aside_session=${encodeURIComponent(token)}`;
    const ws = new WebSocket(url, { headers: { cookie } });
    const first = await new Promise<any>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('no ready frame')), 8000);
      ws.on('message', (raw) => {
        clearTimeout(timer);
