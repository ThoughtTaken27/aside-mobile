...
describe('the follow-up reminder', () => {
  ...
  it('names both tools, in both forms', () => {
    ...
    expect(MOBILE_SESSION_PREAMBLE).toContain('Preserve explanations, examples, caveats and sources');
...
describe('gap 4: continuing from a stuck session', () => {
  ...
  it('rejects a stale question identity instead of answering a different card', async () => {
    parkOnNativeQuestion();
    const res = await app.inject({ method: 'POST', url: '/api/sessions/fixtureAAAA/recover', headers: auth(), payload: { answer: 'Approve', questionId: 'wrong-card' } });
    expect(res.statusCode).toBe(409);
    expect(res.json().error).toBe('no_pending_question');
  });
  ...
  it('seeds a NEW session with the question and the tapped answer', async () => {
    ...
    const repeated = await app.inject({ method: 'POST', url: '/api/sessions/fixtureAAAA/recover', headers: auth(), payload: { answer: 'Approve' } });
    expect(repeated.json().sessionId).toBe(body.sessionId);
