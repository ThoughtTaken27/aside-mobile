...
class FakeChild extends EventEmitter {
  ...
  activity = new PassThrough();
  stdio = [null, this.stdout, this.stderr, this.activity];
...
describe('model and effort resolution', () => {
  it('maps aliases and falls back to config defaults', () => {
    ...
    expect(runner.resolveEffort('max')).toBe('max');
...
describe('daemon-owned stop', () => {
  it('uses the CLI session command and reports its acknowledgement', async () => {
    const runner = makeRunner();
    const result = runner.stopRemote('desktopSess');
    expect(spawned[0].args).toEqual(['session', 'stop', 'desktopSess']);
    children[0].finish(0);
    await expect(result).resolves.toBe(true);
  });

  it('reports a rejected stop without claiming success', async () => {
    const runner = makeRunner();
    const result = runner.stopRemote('idleSess');
    children[0].finish(1);
...
describe('new session creation', () => {
  ...
  it('replays thinking that began before a new session id was discovered', async () => {
    const runner = makeRunner();
    const seen: Array<{ sessionId: string; activity: { phase: string | null } }> = [];
    runner.on('activity', event => seen.push(event));
    const promise = runner.createSession(
      { text: 'hello', model: 'claude-sonnet-5', effort: 'high' },
      { timeoutMs: 3_000, pollMs: 20 },
    );
    // createSession serializes discovery through a promise chain, so allow
    // that chain to spawn its child before driving the fake pipe.
    await new Promise(resolve => setTimeout(resolve, 0));
    // This is the race from the real UI: the signed CLI is already
    // reasoning, but its new directory and therefore its id do not exist.
    children[0].activity.write(JSON.stringify({
      type: 'message_update',
      assistantMessageEvent: { type: 'thinking_start' },
    }) + '\n');
    expect(seen).toEqual([]);
    setTimeout(() => {
      fs.mkdirSync(path.join(env.sessionsDir, '2026-02-01_earlyThinking'));
    }, 60);

    await promise;
    expect(seen).toEqual([{
      sessionId: 'earlyThinking',
      activity: { phase: 'thinking', tools: [], summary: null },
    }]);
    expect(runner.status().inFlight.find(t => t.sessionId === 'earlyThinking')?.activity)
      .toEqual({ phase: 'thinking', tools: [], summary: null });
  });

  it('binds a supported permission on the very first turn', async () => {
    const runner = makeRunner();
    const promise = runner.createSession(
      { text: 'hello', model: 'claude-sonnet-5', effort: 'max' },
      { timeoutMs: 3_000, pollMs: 20, permissionMode: 'full-access' },
    );
    setTimeout(() => {
      fs.mkdirSync(path.join(env.sessionsDir, '2026-02-01_permissionSess'));
    }, 60);
  ...
  it('does not mis-map read-only to a wider CLI permission', async () => {
    const runner = makeRunner();
    const promise = runner.createSession(
      { text: 'hello', model: 'm', effort: 'low' },
      { timeoutMs: 3_000, pollMs: 20, permissionMode: 'read-only' },
    );
    setTimeout(() => {
      fs.mkdirSync(path.join(env.sessionsDir, '2026-02-01_readOnlySess'));
    }, 60);

    await promise;
    expect(spawned[0].args).not.toContain('--permission');
  });
