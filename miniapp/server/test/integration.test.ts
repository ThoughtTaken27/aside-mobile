...
describe('integration smoke', () => {
  ...
  it('pushes daemon model, effort, permission, and run-state changes', async () => {
    ...
    // Quiet transcript: only the daemon row can establish current liveness.
    fs.utimesSync(TRANSCRIPT(), new Date(0), new Date(0));
  ...
  it('waits for a brand new session’s transcript instead of failing', async () => {
    ...
    try {
      ...
      const ws = new WebSocket(
        `${slowBase.replace('http', 'ws')}/ws`,
      );
      const feed = collector(ws);
      await new Promise((resolve) => ws.on('open', resolve));
      ws.send(JSON.stringify({ type: 'auth', token: slowToken }));
      await feed.waitFor((f) => f.type === 'ready');
