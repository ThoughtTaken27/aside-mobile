...
describe('SubagentIndex', () => {
  ...
  it('announces model and effort changes without a status change', async () => {
    let effort = 'medium';
    const index = new SubagentIndex(async () => [{...child('running'), modelLabel: 'Luna', effort}]);
    const seen: string[] = [];
    index.on('updated', id => seen.push(id));
    await index.refresh('p');
    effort = 'high';
    await index.refresh('p');
    expect(seen).toEqual(['p', 'p']);
    expect(index.snapshot('p').children[0].effort).toBe('high');
  });
...
describe('session artifacts', () => {
  ...
  it('refuses bearer tokens in the query string', async () => {
    for (const candidate of [token, 'nope']) {
      const res = await app.inject({
        method: 'GET',
        url: `/api/sessions/fixtureDDDD/artifacts/file?group=artifacts&path=2026-01-06%2Fnotes.md&token=${candidate}`,
      });
      expect(res.statusCode).toBe(401);
      expect(res.json().reason).toBe('token_in_query');
    }
  });
});
