...
describe('GET/POST /api/settings', () => {
  ...
  it('accepts the CLI’s new Max effort and drops invalid permissions', async () => {
    const res = await app.inject({
      method: 'POST',
      url: '/api/settings',
      headers: auth(),
...
describe('POST /api/sessions/:id/stop', () => {
  ...
  it('uses the new CLI command for a live desktop-owned turn', async () => {
    ...
    fs.appendFileSync(
      file,
      `${JSON.stringify({ role: 'assistant', content: [{ type: 'text', text: 'still working' }], timestamp: Date.now() })}\n`,
    );
    ...
    expect(res.statusCode).toBe(200);
    expect(res.json()).toMatchObject({ ok: true, owner: 'desktop' });
  });
...
describe('a suspended session never jams the composer', () => {
  ...
  it('reports the session as suspended on the thread read', async () => {
    ...
    expect(res.json().stoppable).toBe(false);
  });
...
describe('desktop running state remains visible through a quiet transcript', () => {
  beforeEach(() => boot(defaultFixtureRows().map(row => row.id === 'fixtureAAAA' ? {...row, status:'running'} : row)));
  it('trusts the current daemon running status without a recent transcript write', async () => {
    const response = await app.inject({method:'GET',url:'/api/sessions/fixtureAAAA/thread',headers:auth()});
    expect(response.json()).toMatchObject({status:'running',busy:true,stoppable:true,suspended:false});
  });
});
