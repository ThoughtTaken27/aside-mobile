...
describe('the local image file route', () => {
  ...
  it('is behind the same auth as everything else', async () => {
    ...
    // Even a valid bearer is refused in the URL; the client fetches with
    // its Authorization header and renders an ephemeral blob URL instead.
    const validInQuery = await app.inject({
      method: 'GET',
      url: `${fileUrl('fixtureEEEE', shot())}&token=${token}`,
    });
    expect(validInQuery.statusCode).toBe(401);
    expect(validInQuery.json().reason).toBe('token_in_query');
  });
