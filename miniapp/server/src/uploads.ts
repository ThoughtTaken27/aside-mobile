...
export function defaultUploadsDir(): string {
  return (
    ...
    path.join(os.homedir(), '.aside-mobile', 'miniapp-uploads')
  );
}
 ...
 */
/**
 * Delete upload files older than the receipt TTL, including files orphaned
 * by a crash or restart. Symlinks and nested unexpected directories are
 * never followed.
 */
export function sweepUploadDirectory(
  root: string,
  maxAgeMs: number,
  now = Date.now(),
): number {
  ...
  for (const day of days) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // A concurrent delete or unreadable entry recovers next sweep.
      }
    }
    try {
      if (fs.readdirSync(dir).length === 0) fs.rmdirSync(dir);
    } catch {
      // Non-empty or concurrently changed; leave it alone.
    }
  }
  return removed;
}
