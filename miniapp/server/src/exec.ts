...
import { ActivityReader, type LiveActivity } from './activity.js';
...
export interface InFlightTurn {
  /** Sanitized current phase from the live event pipe. */
...
/** A session-control command should acknowledge quickly or get reaped. */
export const SESSION_CONTROL_TIMEOUT_MS = 15_000;
...
/**
 * CLI 1.26.902 can bind Guard or full access while the first turn is being
 * created. `read-only` is still a daemon-only value, so it is applied by the
 * existing post-create update instead of being mapped to a different mode.
 */
export function initialPermissionArgs(mode?: PermissionMode): string[] {
  return mode === 'guard' || mode === 'full-access'
    ? ['--permission', mode]
    : [];
}
...
export class TurnRunner extends EventEmitter {
  ...
  private pump(sessionId: string): void {
    ...
    const args = [
      // Resume inherits the daemon session model/effort. The phone model
      // picker persists changes separately; resume accepts no model flags.
      '--log-dump', '/dev/fd/3',
      'session',
      'resume',
      sessionId,
  ...
  /**
   * Stop a turn owned by the desktop daemon rather than this server.
   *
   * CLI 1.26.902 added the sanctioned `aside session stop <id>` command,
   * backed by the daemon's abort procedure. This replaces the old lie that a
   * Mac-started turn could never be stopped from the phone. The route only
   * calls this after fresh daemon status or transcript liveness confirms activity.
   */
  stopRemote(sessionId: string): Promise<boolean> {
  ...
  private trackChild(
    ...
    const activity = new ActivityReader(value => {
      turn.activity = value;
      if (turn.sessionId) this.emit('activity', { sessionId: turn.sessionId, activity: value });
    });
    // /dev/fd/3 is an inherited pipe on macOS: no raw event file, no
    // credential handling, and no reasoning text forwarded to the phone.
    child.stdio?.[3]?.on('data', (buf: Buffer) => activity.feed(buf.toString('utf8')));
    child.stdio?.[3]?.on('error', () => activity.pause());
    let stderr = '';
    ...
    const finish = (exitCode: number | null, error?: string) => {
      if (settled) return;
      settled = true;
      // The whole row leaves with turn_finished. Clearing here first would
      // flash one frame of generic Working while the turn is already gone.
      turn.activity = { phase: null, tools: [], summary: turn.activity?.summary ?? null };
      clearTimeout(timeout);
      const how = onDone() || {};
  ...
  private async createSessionLocked(
    ...
    const child = this.spawnFn(
      ...
      [
        ...
        '--effort',
        request.effort,
        ...initialPermissionArgs(opts.permissionMode),
    ...
    turn.pending = false;
    dropPending();
    // The CLI can emit thinking before its new session directory appears.
    // ActivityReader retains only the sanitized phase, so replay that truth
    // now instead of losing the first visible state in the discovery race.
    if (turn.activity) {
      this.emit('activity', { sessionId: discovered, activity: turn.activity });
    }
