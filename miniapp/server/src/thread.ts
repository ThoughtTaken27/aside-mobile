...
// Tool detail is collapsed by default. Four thousand characters per call made
// old threads pay hundreds of KB before one detail was opened; 1600 keeps a
// useful diagnostic tail and marks the rest honestly as truncated.
 ...
 * at.
 */
// Base64 screenshots are already compressed and dominated real first paint:
// 4.21 MB of a 5.07 MB thread. Keep the newest normal screenshot within this
// budget; older ones become an explicit count rather than blocking every open.
export const MAX_THREAD_IMAGE_BYTES = 256 * 1024;
...
// Diff cards are also collapsed on first paint. Keep enough context to inspect
// a real edit without shipping several source files inside every thread open.
const FILE_LINE_LIMIT = 120;
