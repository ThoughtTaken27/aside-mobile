...
// A running child is visible progress, so two seconds felt stale next to the
// 80ms parent stream. This stays cached and async, just refreshed sooner.
const RUNNING_TTL_MS = 750;
...
export function toChildSession(
  ...
  return {
    ...
    effort: row.model?.thinkingLevel || null,
    fastMode: row.model?.fastMode ?? false,
...
function stampOf(children: ChildSession[]): string {
  return children.map((c) => `${c.id}:${c.status}:${c.title}:${c.provider}:${c.modelLabel}:${c.effort}:${c.fastMode}`).join('|') || '-';
}
