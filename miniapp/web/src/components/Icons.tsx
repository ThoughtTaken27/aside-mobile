...
export function TodoCircle({
  ...
  if (status === 'completed') {
    return (
      <svg width={size} height={size} viewBox="0 0 16 16" fill="none" aria-hidden>
        {/* The bright brand, not `--success`. This is a filled graphic, so it
            answers to the 3:1 non-text rule rather than the 4.5:1 text one,
            and pointing it at the text clay is what made a completed task
            read as dark text-orange instead of the bright app orange. */}
    ...
    return (
      ...
      >
        ...
        {/*
          ...
          Green is this app's word for FINISHED -- it is the completed
          tick directly above, and the diff gutter's additions. Spending
          it on the in-progress arc as well meant the one list that shows
          both states at once drew them in the same hue, and a spinning
          emerald ring beside a static emerald tick reads as two kinds of
          done. Clay is already what every other live thing in the app
          wears: the running fold's glyph, the activity mark, the context
