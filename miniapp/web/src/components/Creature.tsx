/**
 * The clay-toned blob that identifies a subagent.
 *
 * Sibling agents still need distinct marks, but a rainbow inside an app
 * with one accent looks imported from another product. Slots now vary the
 * intensity of the shared #E87D4F orange against neutral ink instead of changing hue.
 *
...
/**
 * The server still assigns eight stable palette slots. `HUES` remains the
 * slot protocol for backward compatibility; rendering maps those slots to
 * eight clay intensities.
 */
export const HUES = [30, 75, 120, 165, 210, 255, 300, 345];
const CLAY_WEIGHTS = [100, 84, 68, 92, 76, 60, 88, 72] as const;
...
export function Creature({ slot, size = 16 }: { slot: number | undefined; size?: number }) {
  ...
  const index = Math.max(0, HUES.indexOf(hue));
  const body = `color-mix(in oklab, var(--brand) ${CLAY_WEIGHTS[index]}%, var(--foreground))`;
  return (
    <svg
      width={size}
      height={size}
