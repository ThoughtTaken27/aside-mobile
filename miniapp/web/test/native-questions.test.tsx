...
describe('suspended recovery regressions', () => {
  it('allows recovery even when the daemon reports the old session busy', () => {
    ...
    render(<QuestionCard item={nativeItem} onRecover={recover} busy />);
    expect((screen.getByRole('button', { name: /Continue in a new session/ }) as HTMLButtonElement).disabled).toBe(false);
    fireEvent.click(screen.getByRole('button', { name: /Approve/ }));
    expect(recover).toHaveBeenCalledWith('Approve');
  });
  it('associates disabled choices with an explanation', () => {
    render(<QuestionCard item={nativeItem} />);
    const button = screen.getByRole('button', { name: /Approve/ });
    const description = button.getAttribute('aria-describedby');
    expect(description).toBeTruthy();
    expect(document.getElementById(description!)?.textContent).toContain('Respond from Aside');
  });
  it('shows a retryable failure without losing the typed answer', async () => {
    const recover = vi.fn().mockRejectedValue(new Error('offline'));
    render(<QuestionCard item={nativeItem} onRecover={recover} />);
    const input = screen.getByLabelText('Reply to this question') as HTMLInputElement;
    fireEvent.change(input, { target: { value: 'Keep my exact answer' } });
    fireEvent.click(screen.getByRole('button', { name: 'Send' }));
    expect(await screen.findByRole('alert')).toBeTruthy();
    expect(input.value).toBe('Keep my exact answer');
