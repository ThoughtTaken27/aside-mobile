...
describe('C-4 a capped image list says so', () => {
  ...
  /*
   * Step detail now starts collapsed behind its summary line, on every
   * turn including a running one, so these open the run before looking
   * inside it. What is under test is unchanged: a capped image list has
   * to admit what the server left out wherever that list is shown.
   */
  const openRun = (container: HTMLElement) => {
    const row = container.querySelector('.fold-row');
    // Idempotent, because the pin map is module-level and survives
    // `cleanup`: these cases reuse block id `w1`, so a blind click in the
    // second one toggled shut what the first had opened.
    if (row && row.getAttribute('aria-expanded') !== 'true') fireEvent.click(row);
  };

  it('reports the images the server left out', () => {
    const { container } = render(
      fold([step({ images: ['data:image/png;base64,AAA'], imagesDropped: 7 })]),
    );
    openRun(container);
  ...
  it('uses the singular for one dropped image', () => {
    const { container } = render(fold([step({ images: [], imagesDropped: 1 })]));
    openRun(container);
