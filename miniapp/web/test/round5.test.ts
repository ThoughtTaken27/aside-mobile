...
describe('transformCitations', () => {
  ...
  it('drops generic citation scaffolding and keeps only the chip', () => {
    const { markdown, marks } = transformCitations(
      'Leaders. <citation refs="abc">Supporting text quoted from that source</citation> Next.',
      known('abc'),
    );
    expect(markdown).toBe('Leaders. [¹](cite:1) Next.');
    expect(marks).toEqual([{ index: 1, refs: ['abc'], quote: '' }]);
  });
