...
describe('the streaming footer', () => {
  /*
   * This used to assert that the row carried NO timer and NO token count
   * anywhere -- the numbers had been inline in the sentence, where they
   * pushed the purpose sideways and made a status line read as a receipt.
   *
   * They are back, one register down and on their own line. The original
   * requirement was never "no numbers", it was "the sentence is a
   * sentence", so that is what is checked now: the live region holds the
   * purpose and nothing else, and the instrumentation lives outside it.
   */
  it('keeps the purpose sentence clear of the instrumentation', () => {
    const { container } = render(
      <StreamFooter
        phase="working"
        work="tools"
        label="Planning tool status synchronization…"
        tokens={1_240}
      />,
    );
    expect(screen.getByRole('status').textContent)
      .toBe('Planning tool status synchronization…');
    expect(screen.getByRole('status').textContent).not.toMatch(/tokens|\d+s/);

    const meta = container.querySelector('.activity-meta');
    expect(meta?.textContent).toMatch(/0s/);
    expect(meta?.textContent).toMatch(/1.2k tokens/);
    // The mode is stated once in the heading, not repeated below.
    expect(meta?.textContent).not.toMatch(/Working|Thinking|Refining/);
    expect(meta?.querySelector('.activity-mark')).toBeNull();
    expect(container.querySelector('.activity-line .activity-mark')).toBeTruthy();
  });

  it('renders a paced thinking verb before any purpose exists', () => {
    render(<StreamFooter phase="thinking" seed="turn" />);
    const heading = (screen.getByRole('status').textContent ?? '')
      .replace(/…$/, '');
    expect(STATUS_WORDS.thinking[0]).toContain(heading);
  });
});
